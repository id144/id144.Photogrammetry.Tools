...
<javascriptresource>
<name>$$$/JavaScripts/ImageProcessorPNG/Menu=Image Processor PNG...</name>
<category>aaaThisPutsMeAtTheTopOfTheMenu</category>
<eventid>1F9021B3-6045-32E1-AE20-6E504FAA8D50</eventid>
</javascriptresource>
...
function GlobalVariables() {
	...
	strTitle = localize( "$$$/JavaScript/ImageProcessor/Title=Image Processor PNG" );
	...
	strSaveAsPNGHelp = localize( "$$$/JavaScripts/ImageProcessor/SaveAsPNGHelp=Save a file to the PNG format" );
...
function ImageProcessor() {
	...
	this.CreateDialog = function() {
		...
		d.cbConvertICC = p.grpPNG.grpLeft.grpPNG.grpICC.add( 'checkbox', undefined, strConvertICC );
		d.cbConvertICC.helpTip = strConvertICCHelp;
	...
	// I can save in four formats, JPEG, PSD, TIFF, PNG
	this.SaveFile = function ( inFileName, inFolderLocation ) {
		...
		if ( this.params["png"] ) {
			...
			if (app.activeDocument.info.exif != null) {
				//alert(app.activeDocument.info.exif);
				var _exif = app.activeDocument.info.exif;
				for (i = 0; i < _exif.length; i++) {
					 if (_exif[i][0] == "ISO Speed Ratings") {
						ISO = _exif[i][1];
					 }
					 if (_exif[i][0] == "Date Time") {
						DateTime = _exif[i][1].replace(/\:/g,"").replace(" ","_");
					//	alert(DateTime);
					 }

					 if (_exif[i][0] == "F-Stop") {
						fStop = _exif[i][1].replace("/","").replace(".","-").replace("f","");
					 }

					 if (_exif[i][0] == "Exposure Time") {
						Exposure = _exif[i][1].replace("/","-").replace(" ","").replace("sec","");
					 }

				}
			}
			var _prefix = DateTime + "_e" + Exposure + "_f" + fStop + "_iso_"  + ISO + "_";
			var _filename =  _prefix + fileNameNoPath;
			var uniqueFileName = CreateUniqueFileName( subFolderText, _filename, ".png" );
			if ( ! IsFolderWritable( subFolderText ) ) {
				alert( strCannotWriteToFolder + File( subFolderText ).fsName );
			} else {
				SaveAsPNG( uniqueFileName, this.params["q"], this.params["icc"] );
			}
			app.activeDocument.activeHistoryState = historyState;
		}
...
function SaveAsPNG( inFileName, inQuality, inEmbedICC ) {
	var pngOptions = new PNGSaveOptions();
	pngOptions.compression = 9;//inQuality;
